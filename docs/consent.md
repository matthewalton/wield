...
- **Long-term direction, be upfront now:** the project's north star is cross-team skill sharing (Baton #86). Per-person telemetry — cost, usage, attribution — would **never** leave the team under any version of that; the only thing that could ever be shared outward is skill metadata plus aggregated/anonymized adoption signals, and that would be a separate opt-in conversation. This is now an architectural commitment, not just a promise: [ADR-0004](adr/0004-published-aggregates.md) fixes a push model where raw data is never remotely queryable from outside — cross-team sharing is structurally limited to explicitly published team-level aggregates. Saying this now avoids the consent feeling like a bait-and-switch later.
