...
- **[`meta.yaml`](docs/FORMAT.md)** — an optional sidecar file in a skill folder that opts the skill into enriched tracking with team-defined dimensions (e.g. lifecycle stage, author). See [`examples/ticket-planner`](examples/ticket-planner).
...
- **Dashboard** *(planned)* — joins usage with the scanned metadata map: most-used skills, usage per stage, per person.
