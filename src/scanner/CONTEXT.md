...
**Source (of an entry)**:
The sidecar path a map entry came from, relative to the invocation's working
directory. Used in diagnostics to say where a duplicate was first defined.

## Decisions

- **Presence opts in.** A sidecar's existence — not its content — makes a skill
  tracked. An empty sidecar is a valid opt-in.
...
- **The scanner reads sidecars only, today.** FORMAT.md v2 makes frontmatter
  `metadata` the primary home for dimensions with the sidecar as override —
  that is tracked work (Baton #96), and this spec will change under it. Until
  then, sidecar-only is the observed and ratified behaviour.
